...
// -------------------------------------------------------------------------
// NEW: Suppresses console echo/line-input while ffmpeg's progress loop is
// redrawing the screen, so pressing Enter (or any other key) doesn't get
// echoed into the middle of the progress block and corrupt it. Windows only
// (SetConsoleMode is a Win32 API) - on other platforms this is a no-op and
// the original behavior is unchanged.
// -------------------------------------------------------------------------
static class ConsoleInputGuard
{
    ...
    {
        if (!OperatingSystem.IsWindows() || active) return;
        try
        {
            handle = GetStdHandle(STD_INPUT_HANDLE);
            if (GetConsoleMode(handle, out originalMode))
            {
                SetConsoleMode(handle, originalMode & ~(ENABLE_ECHO_INPUT | ENABLE_LINE_INPUT));
                active = true;
            }
        }
        catch
        {
            // If this fails for any reason, just fall back to default console behavior.
        }
    }
...
class Program
{
    ...
    {
        // NEW: hide the cursor and stop echoing keystrokes for the duration of
        // the progress-bar loop, so window resizes / stray keypresses (Enter
        // included) can't corrupt the redraw.
        bool cursorWasVisible = true;
        try { cursorWasVisible = Console.CursorVisible; } catch { }
        try { Console.CursorVisible = false; } catch { }
        ConsoleInputGuard.Suppress();
        ...
        try
        {
            using (Process ffmpeg = new Process())
            {
                ...
                // Reset the progress-bar anchor for this run.
                progressBarStartLine = -1;
                lastKnownWidth = -1;
        ...
        {
            // NEW: always restore console input mode + cursor visibility,
            // even if compression failed or threw.
            ConsoleInputGuard.Restore();
    ...
    // NEW: tracks the console width as of the last draw, so a resize can be detected.
    ...
    {
        // NEW: if the terminal was resized since the last draw, our saved
        // cursor anchor and line-wrap assumptions are no longer valid.
        // Abandon the old position and start a fresh 4-line block below
        // wherever the cursor currently is, instead of writing over stale
        // coordinates.
        int currentWidth;
        try { currentWidth = Console.WindowWidth; } catch { currentWidth = 80; }
        ...
        // Scale the bar to the available width so long lines can't wrap
        // and throw off the fixed 4-line assumption.
        int barWidth = Math.Max(10, Math.Min(50, currentWidth - 30));
        ...
        try
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Console.Write(new string(' ', Math.Min(currentWidth - 1, 120)));
        ...
        {
            // NEW: cursor positioning failed (e.g. window shrank below the
            // saved anchor). Instead of silently staying broken for the rest
            // of the run, drop the anchor so the next tick re-establishes a
            // fresh position rather than repeatedly throwing at a stale one.
            progressBarStartLine = -1;
            lastKnownWidth = -1;
        }
    }
